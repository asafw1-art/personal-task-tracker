...
create policy "Users can read own taxonomy items" on public.task_taxonomy_items for select using (auth.uid() = user_id);
create policy "Users can create own taxonomy items" on public.task_taxonomy_items for insert with check (auth.uid() = user_id);
create policy "Users can update own taxonomy items" on public.task_taxonomy_items for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "Users can delete own taxonomy items" on public.task_taxonomy_items for delete using (auth.uid() = user_id);
